...
function endUpdate() {
	...
	frm.action = "<%=request.getContextPath()%>/community/cBoardUpdate.do";
	frm.method = "post";
	frm.submit();
}
function goBack() {
	location.href="<%=request.getContextPath()%>/community/cBoardList.do";
}
</script>
<c:if test="${not empty cBoardUpdate}">
	<c:forEach items="${cBoardUpdate}">
		alert(${cBoardUpdate});
	</c:forEach>
</c:if>
</head>
