								...
								<table width="100%" class="board-listheader" cellpadding="0"
									...
									<tbody>
										...
										<c:if test="${not empty fBoardList}">
											...
											<c:forEach items="${fBoardList}" var="v" varStatus="s">
												<!--변경 : el태그 - servlet이랑 맞추기 -->
												<tr>
													<td>${v.nno}</td>
													<td><a
			    ...
			    <div class="pagediv">
			        <c:choose>
			            <c:when test="${s.current != 1}">
			                <a href="${ctx}/community/fBoardList.do?pageNum=${prev}"
			        ...
			        <c:forEach varStatus="s" begin="${startPage}" end="${endPage}" step="1">
			            <c:choose>
			                ...
			                <c:otherwise>
			                    <a href="${ctx}/community/fBoardList.do?pageNum=${s.current}"
			        ...
			        <c:choose>
			            <c:when test="${next <= pageCount && pageCount > 1}">
			                <a href="${ctx}/community/fBoardList.do?pageNum=${endPage}"
