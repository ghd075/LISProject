...
function endUpdate() {
	...
	frm.action = "<%=request.getContextPath()%>/community/uBoardUpdate.do";
	frm.method = "post";
	frm.submit();
}
function goBack() {
	location.href="<%=request.getContextPath()%>/community/uBoardList.do";
}
</script>
<c:if test="${not empty uBoardUpdate}">
	<c:forEach items="${uBoardUpdate}">
		alert(${uBoardUpdate});
	</c:forEach>
</c:if>
</head>
