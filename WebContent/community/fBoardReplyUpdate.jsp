...
function endUpdate(rno,nno) 
{
	var frm = document.qnaReply;
		frm.method = "post";
		frm.action = "<%= request.getContextPath() %>/community/fBoardReplyUpdate.do?rno="+rno+"&nno=" +nno;
		frm.submit();
}	


function goBack() {
	location.href="<%=request.getContextPath()%>/community/fBoardDetail.do?nno=${selReply.nno}";
}
