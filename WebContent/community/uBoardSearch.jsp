										...
										<c:if test="${not empty searchList}">
											...
											<c:forEach items="${searchList}" var="v" varStatus="s">
												...
												<tr>
													<td>${v.nno}</td>
													<td><a
														href="${ctx}/community/uBoardDetail.do?nno=${v.nno}&pageNum=${s.count}">${v.ntitle}</a></td>
			    ...
			    <div class="pagediv">
			        <c:choose>
			            <c:when test="${startPage != 1}">
			                <a href="${ctx}/community/uBoardSearch.do?pageNum=${prev}" class="layout-pagination-button layout-back"><</a>
			            </c:when>
			            <c:otherwise>
			            ...
			            <c:otherwise>
			                <c:forEach varStatus="s" begin="${startPage}" end="${endPage}" step="1">
			                    <c:choose>
			                        ...
			                        <c:otherwise>
			                            <a href="${ctx}/community/uBoardSearch.do?pageNum=${s.current}&nSearch=${nSearch}" class="layout-pagination-button">${s.current}</a>
			                        </c:otherwise>
			                    </c:choose>
			                </c:forEach>
			            </c:otherwise>
			        ...
			        <c:choose>
			            <c:when test="${next <= pageCount && pageCount > 1}">
			                <a href="${ctx}/community/uBoardSearch.do?pageNum=${next}&msearchbar=${msearchbar}" class="layout-pagination-button layout-next">></a>
			            </c:when>
			            <c:otherwise>
							...
							<c:if test="${member.authority eq 1}">
								<div id="nWritebtn">
									<button type="button" id="nWrite" name="nWrite"
										onclick="location.href='${ctx}/community/uBoardWrite.jsp'">글쓰기</button>
								</div>
							</c:if>
						</c:if></td>
					<td>
						<div class="search_bt">
							<form name="noticeSearch" action="${ctx}/community/uBoardSearch.do">
