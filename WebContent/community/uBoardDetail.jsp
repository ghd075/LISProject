...
function goSearch(){
	var frm = document.noticeSearch;
		frm.action="<%=request.getContextPath()%>/community/uBoardSearch.do";
		frm.method = "post";
		frm.submit();
	};
...
function goDelete() {
		...
		if (confirm("정말 삭제하시겠습니까??") == true){    //확인
			frm.method = "post";
			frm.action = "<%=request.getContextPath()%>/community/uBoardDelete.do";
			frm.submit();
		}else{   //취소
		    return;
		}
...
function startUpdate() {
	var frm = document.noticeDetail;
	frm.method = "get";
	frm.action = "<%=request.getContextPath()%>/community/uBoardUpdate.jsp";
	frm.submit();
}
		...
		<div id="c_nav">
			...
			<div class="inner_nav">
				<ul class="list_menu">
					...
					<li class="list-item active"><a href="${ctx}/community/uBoardList.do" style="background: #FFFDE4;">중고장터<span
							class="aside_arrow"><i class="fa fa-caret-right"></i></span></a></li>
					...
					<table>
						...
						<tr>
							...
							<td style="padding-left: 8px;" id="ndate"  name="ndate"><fmt:formatDate
									value="${uBdetail.ndate}" pattern="yyyy-MM-dd" var="date" /> <input
								type="hidden" name="ndate" value="${date}"> ${date}</td>
							<td class="qna_title2 qna_box">조회수</td>
							<td id="nview" name="nview">
							<input type="hidden" name="nview" value="${uBdetail.nview}"> ${uBdetail.nview}
							</td>
						...
						<tr>
							<td colspan="4" id="ncontent" name="ncontent"
								...
								value="${uBdetail.ncontent}">
								<p style="white-space: pre-line;">&nbsp;&nbsp;&nbsp;&nbsp; ${uBdetail.ncontent}</p>
								</td>
						</tr>
					</table>
					<div class="b_button2">
						<c:if test="${not empty member}">
							<c:if test="${member.authority eq 1 || (member.authority eq 0 && uBdetail.mid == member.mid)}">
						...
						<button type="button" class="b_button" onclick="location.href='<%=request.getContextPath()%>/community/uBoardList.do'">글목록</button>
					</div>
				</div>
			</form>			
		</div>
