						...
						<td>
							<div class="section_t">
								<table width="100%" class="board-listheader" cellpadding="0"
									...
									<tbody>
		                          										<!--목록 뿌리기 -->
										<c:if test="${not empty cBoardList}">
											<!--변경 : el태그 - servlet이랑 맞추기 -->
											<c:forEach items="${cBoardList}" var="v" varStatus="s">
												<!--변경 : el태그 - servlet이랑 맞추기 -->
												<tr>
													<td>${v.nno}</td>
													<td><a
			    ...
			    <div class="pagediv">
			        <c:choose>
			           <c:when test="${s.current != 1}">
			                <a href="${ctx}/community/cBoardList.do?pageNum=${prev}"
			        ...
			        <c:forEach varStatus="s" begin="${startPage}" end="${endPage}" step="1">
			            <c:choose>
			                ...
			                <c:otherwise>
			                    <a href="${ctx}/community/cBoardList.do?pageNum=${s.current}"
			        ...
			        <c:choose>
			            <c:when test="${next <= pageCount && pageCount > 1}">
			                <a href="${ctx}/community/cBoardList.do?pageNum=${endPage}"
	...
	<script type="text/javascript">
	    ...
	    function postInsert(){
	          if(${mno} == 0){
	             ...
	             window.location="postInsert.do?group="+${group}+"&&cust_no="+${mno};
	          }
	       }	
	</script>
</body>
