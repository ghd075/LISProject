	...
	<div id="c_main">
		...
		<div class="page_section">
			<div class="head_aticle">
				<h2 class="tit">
					묻고 답하기 <span class="tit_sub"> ' ${nSearch} ' (으)로 검색한 결과입니다. </span>
				</h2>
			</div>
			    ...
			    <div class="pagediv">
			        <c:choose>
			            <c:when test="${startPage != 1}">
			                <a href="${ctx}/intro/qnaSearch.do?pageNum=${prev}" class="layout-pagination-button layout-back"><</a>
			            </c:when>
			            <c:otherwise>
			            ...
			            <c:otherwise>
			                <c:forEach varStatus="s" begin="${startPage}" end="${endPage}" step="1">
			                    <c:choose>
			                        ...
			                        <c:otherwise>
			                            <a href="${ctx}/intro/qnaSearch.do?pageNum=${s.current}&nSearch=${nSearch}" class="layout-pagination-button">${s.current}</a>
			                        </c:otherwise>
			                    </c:choose>
			                </c:forEach>
			            </c:otherwise>
			        ...
			        <c:choose>
			            <c:when test="${next <= pageCount && pageCount > 1}">
			                <a href="${ctx}/intro/qnaSearch.do?pageNum=${next}&msearchbar=${msearchbar}" class="layout-pagination-button layout-next">></a>
			            </c:when>
			            <c:otherwise>
		...
		<table class="board-search board-search2">
			<tbody>
				<tr>
					...
					<td>
						<div class="search_bt">
							<form name="qnaSearch" action="${ctx}/intro/qnaSearch.do">
