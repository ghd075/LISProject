			    ...
			    <div class="pagediv">
			        <c:choose>
			            <c:when test="${startPage != 1}">
			                <a href="${ctx}/intro/noticeSearch.do?pageNum=${prev}" class="layout-pagination-button layout-back"><</a>
			            </c:when>
			            <c:otherwise>
			            ...
			            <c:otherwise>
			                <c:forEach varStatus="s" begin="${startPage}" end="${endPage}" step="1">
			                    <c:choose>
			                        ...
			                        <c:otherwise>
			                            <a href="${ctx}/intro/noticeSearch.do?pageNum=${s.current}&nSearch=${nSearch}" class="layout-pagination-button">${s.current}</a>
			                        </c:otherwise>
			                    </c:choose>
			                </c:forEach>
			            </c:otherwise>
			        ...
			        <c:choose>
			            <c:when test="${next <= pageCount && pageCount > 1}">
			                <a href="${ctx}/intro/noticeSearch.do?pageNum=${next}&msearchbar=${msearchbar}" class="layout-pagination-button layout-next">></a>
			            </c:when>
			            <c:otherwise>
