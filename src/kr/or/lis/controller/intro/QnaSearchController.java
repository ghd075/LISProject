...
public class QnaSearchController implements Controller {
	...
	public String requestHandler(HttpServletRequest request, HttpServletResponse response)
		...
		// 총 글 개수
		int nCount = qnaService.getSearchCount(nSearch); /* 변경 : 메소드 */
...
//		    session.setAttribute("search_error", "검색어를 입력해주세요");
		    ...
		    nextPage = "redirect:" + ctx + "/intro/qnaList.do";
		} else {
		    // 검색 결과가 있을 때, search_error 속성을 삭제
//			HttpSession session = request.getSession();
//		    session.removeAttribute("search_error");
		    
			List<NoticeVO> searchList = qnaService.noticeSearch(nSearch, startRnum, endRnum);
			...
			nextPage = "qnaSearch"; 		// 포워딩할 JSP 페이지 경로
		}
